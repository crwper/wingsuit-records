...
CREATE OR REPLACE FUNCTION "public"."create_sequence"("p_title" "text", "p_base_flyer_id" "text" DEFAULT 'F1'::"text") RETURNS "uuid"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
declare v_id uuid;
begin
  ...
  insert into public.sequences (title, owner_user_id, base_flyer_id)
  values (trim(p_title), auth.uid(), coalesce(nullif(trim(p_base_flyer_id), ''), 'F1'))
  returning id into v_id;

  -- Owner membership is added by trigger
  return v_id;
end$$;


ALTER FUNCTION "public"."create_sequence"("p_title" "text", "p_base_flyer_id" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."save_sequence_roster"("p_sequence_id" "uuid", "p_roster" "jsonb", "p_base_flyer_id" "text") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  select owner_user_id into v_owner
  from public.sequences where id = p_sequence_id;
  ...
  -- Compute count and "base present?" inside ONE CTE scope
  with raw as (
  ...
  select
    count(*)::int as c,
    bool_or(flyer_id = trim(p_base_flyer_id)) as has_base
  into v_count, v_has_base
  from cleaned;

  if v_count = 0 then
    raise exception 'roster cannot be empty';
  end if;
  if coalesce(trim(p_base_flyer_id),'') = '' then
    raise exception 'base_flyer_id required';
  end if;
  if not v_has_base then
    raise exception 'base_flyer_id must be in roster';
  end if;

  -- Replace roster (recompute cleaned in-line for the insert)
  ...
  set base_flyer_id = trim(p_base_flyer_id)
  where id = p_sequence_id;
end
$$;


ALTER FUNCTION "public"."save_sequence_roster"("p_sequence_id" "uuid", "p_roster" "jsonb", "p_base_flyer_id" "text") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."sequences" (
    ...
    "title" "text" NOT NULL,
    "owner_user_id" "uuid" NOT NULL,
    "base_flyer_id" "text" DEFAULT 'F1'::"text" NOT NULL,
...
GRANT ALL ON FUNCTION "public"."create_sequence"("p_title" "text", "p_base_flyer_id" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."create_sequence"("p_title" "text", "p_base_flyer_id" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_sequence"("p_title" "text", "p_base_flyer_id" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."save_sequence_roster"("p_sequence_id" "uuid", "p_roster" "jsonb", "p_base_flyer_id" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."save_sequence_roster"("p_sequence_id" "uuid", "p_roster" "jsonb", "p_base_flyer_id" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."save_sequence_roster"("p_sequence_id" "uuid", "p_roster" "jsonb", "p_base_flyer_id" "text") TO "service_role";
